...
async function whatsAsena () {
    ...
    setInterval(async () => { 
        ...
        await axios.get('https://gist.github.com/Ramanan/68b7d4455dd84d27cd1daa5bd30eb2ca/raw/').then(async (ann) => {
    ...
    conn.on('message-new', async msg => {
        ...
        if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            ...
            if (gb !== false) {
                ...
                var ramanan = await conn.groupMetadata(msg.key.remoteJid)
                await conn.sendMessage(msg.key.remoteJid, Buffer.from(ramananser.data),MessageType.video, {mimetype: Mimetype.gif,thumbnail: fs.readFileSync('./image/wel.jpg'), caption: gb.message.replace('{gif}', '').replace('{gphead}', ramanan.subject).replace('{gpmaker}', ramanan.owner).replace('{gpdesc}', ramanan.desc).replace('{owner}', conn.user.name) });
            } else {
                var pinkjson = await conn.groupMetadata(msg.key.remoteJid)
...
*/
                       ...
                       try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ RamaniMwol ~______~*' +
